...
def generar_reporte_cuentas_pendientes(request):
    """Genera un PDF con el reporte de cuentas pendientes aplicando los filtros usando xhtml2pdf"""
    try:
        ...
        total_deuda_bs_sum = Decimal('0')
        total_deuda_usd_sum = Decimal('0')
        ...
        for cliente in clientes_credito:
            ...
            if ventas_cliente_credito.exists():
                ...
                # Calcular días desde venta más antigua
                dias_transcurridos = 0
                venta_mas_antigua = ventas_pendientes.order_by('Fecha_Venta').first()
                if venta_mas_antigua:
                    diferencia = hoy - venta_mas_antigua.Fecha_Venta
                    dias_transcurridos = diferencia.days
                
                # Aplicar filtro de estado (mora)
                if estado:
                ...
                clientes_deuda_detallada.append({
                    ...
                    'deuda_total_usd_display': format_number_es(deuda_total_usd),
                    'deuda_total_bs_display': format_number_es(deuda_total_bs),
                    'ventas_pendientes': ventas_pendientes.count(),
                    'dias_transcurridos': dias_transcurridos,
                })
                
                total_deuda_bs_sum += deuda_total_bs
                total_deuda_usd_sum += deuda_total_usd
        
        # Ordenar por deuda total (descendente)
        clientes_deuda_detallada.sort(key=lambda x: x['deuda_total_usd'], reverse=True)
        
        # Construir info de filtros
        filtros_parts = []
        if query: filtros_parts.append(f"Búsqueda: '{query}'")
        if estado: filtros_parts.append(f"Estado: {estado}")
        if tipo_cliente: filtros_parts.append(f"Tipo: {tipo_cliente}")
        ...
        context = {
            'clientes_deuda': clientes_deuda_detallada,
            'total_clientes': len(clientes_deuda_detallada),
            'total_bs': format_number_es(total_deuda_bs_sum),
            'total_usd': format_number_es(total_deuda_usd_sum),
            'logo_url': logo_url,
            'filtros_info': filtros_info,
            'request': request,
        }
        
        html_string = render_to_string('cuentas_pendientes/reporte_pdf.html', context)
        
        response = HttpResponse(content_type='application/pdf')
        filename = "reporte_cuentas_pendientes.pdf"
